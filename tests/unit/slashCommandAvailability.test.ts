...
describe("isSlashCommandListEnabled", () => {
  ...
  it("returns false for codex before session_active", () => {
    ...
    expect(
      isSlashCommandListEnabled({ conversationType: "codex", codexStatus: "connecting" }),
    ).toBe(false);
    expect(isSlashCommandListEnabled({ conversationType: "codex", codexStatus: "connected" })).toBe(
      false,
    );
  });
  ...
  it("returns true for codex when session is active", () => {
    expect(
      isSlashCommandListEnabled({ conversationType: "codex", codexStatus: "session_active" }),
    ).toBe(true);
  });

...
function findAppAsarUnderOut(): string | null {
  ...
  const preferred = asarFiles.find((file) =>
    /(?:^|\\|\/)(win-unpacked|linux-unpacked|mac|mac-arm64|mac-x64)(?:\\|\/)/.test(file),
  );
...
function getAsarEntries(asarPath: string): Set<string> {
  const candidates =
    process.platform === "win32" ? ["bunx.cmd", "bunx", "npx.cmd", "npx"] : ["bunx", "npx"];
  let output = "";

  for (const cmd of candidates) {
    try {
      const args = cmd.startsWith("bunx")
        ? ["--bun", "asar", "list", asarPath]

...
export async function resolveFileRefs(
  ...
  if (Array.isArray(obj)) {
    const results = await Promise.all(
      obj.map((item) => resolveFileRefs(item, extensionDir, visited)),
    );
    return results;
  }
  if (obj !== null && typeof obj === "object") {
    ...
    const resolved = await Promise.all(
      entries.map(async ([key, value]) => [
        key,
        await resolveFileRefs(value, extensionDir, visited),
      ]),
    );
...
async function resolveFileRefValue(
  ref: string,
  extensionDir: string,
  visited: Set<string>,
): Promise<unknown> {
  ...
  if (!existsSync(absolutePath)) {
    console.warn(
      `[Extensions] Referenced file not found: ${absolutePath} (from $file:${relativePath})`,
    );
    return ref;
  }

...
export const ExtensionMetaSchema = z
  .object({
    ...
    version: z
      .string()
      .regex(/^\d+\.\d+\.\d+(-[\w.]+)?$/, "Version must be semver format (e.g., 1.0.0)"),
    ...
    dependencies: z
      .record(
        ...
        z
          .string()
          .regex(/^\^?\d+\.\d+\.\d+(-[\w.]+)?$/, "Dependency version must be semver format"),
      )
      .optional()
...
function validateContributeIds(
  contributes: z.infer<typeof ExtContributesSchemaBase>,
): string | true {

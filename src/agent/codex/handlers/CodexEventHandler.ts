...
export class CodexEventHandler {
  ...
  private processCodexEvent(msg: CodexEventMsg) {
    ...
    if (
      this.isMessageType(msg, "exec_approval_request") ||
      this.isMessageType(msg, "apply_patch_approval_request")
    ) {
  ...
  private processApplyPatchRequest(
    ...
    console.log(
      `[CodexEventHandler] patch needs confirmation: ${unifiedRequestId}, files=${files.join(", ")}`,
    );
    ...
    const description =
      msg.message || `${displayInfo.icon} Codex wants to apply proposed code changes`;

...
class AcpDetector {
    ...
    try {
      ...
      for (const item of adapters) {
        ...
        const defaultCliPath =
          typeof adapter.defaultCliPath === "string" ? adapter.defaultCliPath : undefined;
        ...
        const extensionName =
          typeof adapter._extensionName === "string" ? adapter._extensionName : "unknown-extension";
  ...
  private async addCustomAgentsToList(detected: DetectedAgent[]): Promise<void> {
    try {
      ...
      const enabledAgents = customAgents.filter(
        (agent) => agent.enabled && (agent.defaultCliPath || agent.isPreset),
      );
  ...
  async refreshCustomAgents(): Promise<void> {
    ...
    this.detectedAgents = this.detectedAgents.filter(

...
export default forkTask(({ data }, pipe) => {
  ...
  console.log(
    `[GeminiWorker] presetRules preview: ${data.presetRules?.substring(0, 200) || "empty"}`,
  );
        ...
        event.data = (event.data as any[]).map((tool: any) => {
          ...
          if (confirmationDetails) {
            ...
            if (!registeredConfirmCallIds.has(tool.callId)) {
              ...
              pipe.once(
                tool.callId,
                (confirmKey: string, deferred?: { resolve: (v: unknown) => void }) => {
  ...
  pipe.on(
    "send.message",
    (event: { input: string; msg_id: string; files?: string[] }, deferred) => {

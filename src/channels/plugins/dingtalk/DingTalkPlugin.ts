...
export class DingTalkPlugin extends BasePlugin {
  ...
  async editMessage(
    ...
    const truncatedText =
      text.length > DINGTALK_MESSAGE_LIMIT
        ? text.slice(0, DINGTALK_MESSAGE_LIMIT - 3) + "..."
        : text;
  ...
  private async handleCardCallback(data: any, streamMessageId: string): Promise<void> {
    try {
      ...
      if (
        actionInfo.name === "system.confirm" &&
   ...
   */
  private async createAndDeliverAICard(
    chatType: "user" | "group",
    id: string,
    _initialText: string,
  ): Promise<string> {
    ...
    const openSpaceId =
      chatType === "group" ? `dtv1.card//IM_GROUP.${id}` : `dtv1.card//IM_ROBOT.${id}`;

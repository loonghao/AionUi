...
} from "@/renderer/hooks/useSlashCommandController";
...
const SendBox: React.FC<{
  ...
  const { setSendBoxHandler, domSnippets, removeDomSnippet, clearDomSnippets } =
    usePreviewContext();
  ...
  useEffect(() => {
    ...
    slashController.activeIndex,
    slashController.isOpen,
    slashController.filteredCommands.length,
  ]);
      ...
      >
        {slashController.isOpen && (
          <div
            ...
            className="absolute left-0 right-0 bottom-[calc(100%+8px)] z-50 max-h-220px overflow-auto rounded-12px border border-solid border-[var(--color-border-2)] bg-[var(--color-bg-2)] shadow-lg p-6px flex flex-col gap-4px"
          >

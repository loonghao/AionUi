      ...
      try {
        ...
        if (cachedInfo?.availableModels?.length > 0) {
          ...
          setModelInfo({
            ...
            currentModelLabel:
              (effectiveModelId &&
                cachedInfo.availableModels.find((m) => m.id === effectiveModelId)?.label) ||
              effectiveModelId,
          });
        }
      } catch {
    ...
    const handler = (message: IResponseMessage) => {
      ...
      if (message.type === "acp_model_info" && message.data) {
        ...
        if (
          ...
          incoming.availableModels?.length > 0
        ) {

...
const ModalMcpManagementSection: React.FC<{ message: MessageInstance; isPageMode?: boolean }> = ({
    ...
    const loadAgents = async () => {
      try {
        ...
        if (response.success && response.data) {
          setDetectedAgents(
            response.data.map((agent) => ({ backend: agent.backend, name: agent.name })),
          );
        }
      } catch (error) {
  ...
  useEffect(() => {
    const httpServers = mcpServers.filter(
      (s) => s.transport.type === "http" || s.transport.type === "sse",
    );
...
const ToolsModalContent: React.FC = () => {
  ...
  const handleImageGenerationModelChange = (
                  ...
                  >
                    {imageGenerationModelList.map(({ model, ...platform }) => (
                      <AionSelect.OptGroup label={platform.name} key={platform.id}>
                        {model.map((modelName) => (
                          <AionSelect.Option
                            key={platform.id + modelName}
                            value={platform.id + "|" + modelName}
                          >
                            {modelName}
                          </AionSelect.Option>
                        ))}

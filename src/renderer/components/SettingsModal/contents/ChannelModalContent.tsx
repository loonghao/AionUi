...
const useChannelModelSelection = (configKey: ChannelModelConfigKey): GeminiModelSelection => {
  ...
  const [resolvedInitialModel, setResolvedInitialModel] = useState<TProviderWithModel | undefined>(
    undefined,
  );
  ...
  useEffect(() => {
    ...
    const restore = async () => {
      try {
        const saved = (await ConfigStorage.get(configKey)) as
          | { id: string; useModel: string }
          | undefined;
    ...
    async (provider: IProvider, modelName: string) => {
      try {
        ...
        await channel.syncChannelSettings
          ...
          .catch((err) =>
            console.warn(`[ChannelSettings] syncChannelSettings failed for ${platform}:`, err),
          );
...
const ChannelModalContent: React.FC = () => {
  ...
  const [dingtalkPluginStatus, setDingtalkPluginStatus] = useState<IChannelPluginStatus | null>(
    null,
  );
    ...
    try {
      if (enabled) {
        ...
        if (result.success) {
          ...
          Message.error(
            result.msg || t("settings.assistant.enableFailed", "Failed to enable plugin"),
          );
        }
      } else {
        ...
        if (result.success) {
          ...
          Message.error(
            result.msg || t("settings.assistant.disableFailed", "Failed to disable plugin"),
          );
        }
      }
    } catch (error: any) {
    ...
    try {
      if (enabled) {
        ...
        if (!larkPluginStatus?.hasToken) {
          Message.warning(
            t("settings.lark.credentialsRequired", "Please configure Lark credentials first"),
          );
    ...
    try {
      if (enabled) {
        if (!dingtalkPluginStatus?.hasToken) {
          Message.warning(
            t(
              ...
              "Please configure DingTalk credentials first",
            ),
          );
        ...
        if (result.success) {
          ...
          Message.error(
            result.msg || t("settings.dingtalk.disableFailed", "Failed to disable DingTalk plugin"),
          );
        }
      }
    } catch (error: any) {
  ...
  return (
    <AionScrollArea className={isPageMode ? "h-full" : ""}>
      <div className="px-[12px] md:px-[28px]">
        <h2 className="text-20px font-500 text-t-primary m-0">

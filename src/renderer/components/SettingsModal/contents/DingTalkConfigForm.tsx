...
const DingTalkConfigForm: React.FC<DingTalkConfigFormProps> = ({
      ...
      try {
        ...
        if (agentsResp.success && agentsResp.data) {
          ...
          const list = agentsResp.data
            .filter((a) => !a.isPreset && CHANNEL_SUPPORTED_BACKENDS.has(a.backend))
            .map((a) => ({
              backend: a.backend,
              name: a.name,
  ...
  const handleTestConnection = async () => {
    ...
    if (!clientId.trim() || !clientSecret.trim()) {
      Message.warning(
        t("settings.dingtalk.credentialsRequired", "Please enter Client ID and Client Secret"),
      );
      return;
    }
    ...
    try {
      ...
      if (result.success && result.data?.success) {
        ...
        setCredentialsTested(false);
        Message.error(
          result.data?.error || t("settings.dingtalk.connectionFailed", "Connection failed"),
        );
      }
    } catch (error: any) {
      setCredentialsTested(false);
  ...
  const handleAutoEnable = async () => {
    try {
      ...
      if (result.success) {
        ...
        Message.error(
          result.msg || t("settings.dingtalk.enableFailed", "Failed to enable DingTalk plugin"),
        );
      }
    } catch (error: any) {
      console.error("[DingTalkConfig] Auto-enable failed:", error);
      Message.error(
        error.message || t("settings.dingtalk.enableFailed", "Failed to enable DingTalk plugin"),
      );
    }
  };
  ...
  const handleApprovePairing = async (code: string) => {
    try {
      ...
      if (result.success) {
        ...
        Message.error(
          result.msg || t("settings.assistant.approveFailed", "Failed to approve pairing"),
        );
      }
    } catch (error: any) {
  ...
  const handleRejectPairing = async (code: string) => {
    try {
      ...
      if (result.success) {
        ...
        Message.error(
          result.msg || t("settings.assistant.rejectFailed", "Failed to reject pairing"),
        );
      }
    } catch (error: any) {
      Message.error(error.message);
    ...
    <div className="flex flex-col gap-24px">
      ...
      >
        {hasExistingUsers ? (
          ...
          >
            <span>
              <Input
                ...
                placeholder={
                  hasExistingUsers || pluginStatus?.hasToken ? "••••••••••••••••" : "dingxxxxxxxxxx"
                }
                style={{ width: 240 }}
                status={
                  touched.clientId && !clientId.trim() && !pluginStatus?.hasToken
                    ? "error"
                    : undefined
                }
          ...
          <Input
            ...
            style={{ width: 240 }}
            status={
              touched.clientId && !clientId.trim() && !pluginStatus?.hasToken ? "error" : undefined
            }
            disabled={hasExistingUsers}
          />
        )}
      ...
      >
        {hasExistingUsers ? (
          ...
          >
            <span>
              <Input.Password
                ...
                placeholder={
                  hasExistingUsers || pluginStatus?.hasToken
                    ? "••••••••••••••••"
                    : "xxxxxxxxxxxxxxxxxx"
                }
                style={{ width: 240 }}
                status={
                  touched.clientSecret && !clientSecret.trim() && !pluginStatus?.hasToken
                    ? "error"
                    : undefined
                }
                visibilityToggle
          ...
          <Input.Password
            ...
            onBlur={() => setTouched((prev) => ({ ...prev, clientSecret: true }))}
            placeholder={
              hasExistingUsers || pluginStatus?.hasToken ? "••••••••••••••••" : "xxxxxxxxxxxxxxxxxx"
            }
            style={{ width: 240 }}
            status={
              touched.clientSecret && !clientSecret.trim() && !pluginStatus?.hasToken
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-6px">
                        <span>{a.name}</span>
                        {a.isExtension && (
                          <span className="text-[10px] leading-none px-4px py-2px rd-8px bg-[var(--color-primary)] text-white">
                            Ext
                          </span>
                        )}
                      </div>
      ...
      {pluginStatus?.enabled && authorizedUsers.length === 0 && (
        ...
        >
          ...
          />
          {pluginStatus?.error && (
            <div className="text-14px text-red-600 dark:text-red-400 mb-12px">
              {pluginStatus.error}
            </div>
          )}
          {pluginStatus?.connected && (
            <div className="text-14px text-t-secondary space-y-8px">
              ...
              <p className="m-0">
                <strong>1.</strong>{" "}
                {t("settings.dingtalk.step1", "Open DingTalk and find your bot application")}
              </p>
              <p className="m-0">
                <strong>2.</strong>{" "}
                {t("settings.dingtalk.step2", "Send any message to initiate pairing")}
              </p>
              <p className="m-0">
                <strong>3.</strong>{" "}
                {t(
          ...
          {!pluginStatus?.connected && !pluginStatus?.error && (
            <div className="text-14px text-t-secondary">
              {t(
                ...
                "Connection is being established. Please wait...",
              )}
            </div>
          )}
        </div>
      ...
      {pluginStatus?.enabled && authorizedUsers.length === 0 && (
        <div className="bg-fill-1 rd-12px pt-16px pr-16px pb-16px pl-0">
          ...
          {pairingLoading ? (
            ...
            <Empty
              description={t("settings.assistant.noPendingPairings", "No pending pairing requests")}
            />
          ) : (
            <div className="flex flex-col gap-12px">
              {pendingPairings.map((pairing) => (
                <div
                  key={pairing.code}
                  className="flex items-center justify-between bg-fill-2 rd-8px p-12px"
                >
                  <div className="flex-1">
                    <div className="flex items-center gap-8px">
      ...
      {authorizedUsers.length > 0 && (
        <div className="bg-fill-1 rd-12px pt-16px pr-16px pb-16px pl-0">
          ...
          {usersLoading ? (
            ...
            <Empty
              description={t("settings.assistant.noAuthorizedUsers", "No authorized users yet")}
            />
          ) : (
            <div className="flex flex-col gap-12px">

  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      if (result.success && result.data?.success) {
        ...
        setCredentialsTested(false);
        Message.error(
          result.data?.error || t("settings.lark.connectionFailed", "Connection failed"),
        );
      }
    } catch (error: any) {
      setCredentialsTested(false);
  ...
  const handleAutoEnable = async () => {
    try {
      ...
      if (result.success) {
        ...
        Message.error(
          result.msg || t("settings.lark.enableFailed", "Failed to enable Lark plugin"),
        );
      }
    } catch (error: any) {
      console.error("[LarkConfig] Auto-enable failed:", error);
      Message.error(
        error.message || t("settings.lark.enableFailed", "Failed to enable Lark plugin"),
      );
    }
  };
    ...
    <div className="flex flex-col gap-24px">
        ...
        {hasExistingUsers ? (
          <Tooltip
            content={t(
              ...
              "请先关闭 Channel 并删除所有已授权用户后，再尝试修改",
            )}
          >
            <span>
              <Input
                ...
                placeholder={
                  hasExistingUsers || pluginStatus?.hasToken ? "••••••••••••••••" : "cli_xxxxxxxxxx"
                }
                style={{ width: 240 }}
                status={
                  touched.appId && !appId.trim() && !pluginStatus?.hasToken ? "error" : undefined
                }
          ...
          >
            <span>
              <Input.Password
                ...
                style={{ width: 240 }}
                status={
                  touched.appSecret && !appSecret.trim() && !pluginStatus?.hasToken
                    ? "error"
                    : undefined
                }
                visibilityToggle
      ...
      >
        <GeminiModelSelector
          ...
          label={
            !isGeminiAgent
              ? t("settings.assistant.autoFollowCliModel", "自动跟随CLI运行时的模型")
              : undefined
          }
          variant="settings"
        />
        ...
        >
          ...
          {pluginStatus?.connected && (
            <div className="text-14px text-t-secondary space-y-8px">
              ...
              <p className="m-0">
                <strong>1.</strong>{" "}
                {t("settings.lark.step1", "Open Feishu/Lark and find your bot application")}
              </p>
              <p className="m-0">
                <strong>2.</strong>{" "}
                {t("settings.lark.step2", "Send any message to initiate pairing")}
              </p>
              <p className="m-0">
                <strong>3.</strong>{" "}
                {t(
              ...
              <p className="m-0">
                ...
                {t(
                  "settings.lark.step4",
                  "Once approved, you can start chatting with the AI assistant through Lark!",
                )}
              </p>
            </div>
          )}
          {!pluginStatus?.connected && !pluginStatus?.error && (
            <div className="text-14px text-t-secondary">
              {t(
                ...
                "WebSocket connection is being established. Please wait...",
              )}
            </div>
          )}
        </div>

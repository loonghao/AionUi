  ...
  return useCallback(
    (uploadFile: string[] | ((prev: string[]) => string[])) => {
      mutate((prev) => {
        ...
        const previousUploadFile = Array.isArray(prev?.uploadFile)
          ? (prev?.uploadFile as string[])
          : [];
        const newUploadFile =
          typeof uploadFile === "function" ? uploadFile(previousUploadFile) : uploadFile;
 ...
 */
export const useSendBoxFiles = ({
  atPath,
  uploadFile,
  setAtPath,
  setUploadFile,
}: UseSendBoxFilesProps) => {
  // 处理拖拽或粘贴的文件
  ...
  const processMessageWithFiles = useCallback(
    (message: string): string => {
      if (atPath.length || uploadFile.length) {
        const cleanUploadFiles = getCleanFileNames(uploadFile).map((fileName) =>
          formatFileRef(fileName),
        );
        ...
        const cleanAtPaths = getCleanFileNames(atPathStrings).map((fileName) =>
          formatFileRef(fileName),
        );

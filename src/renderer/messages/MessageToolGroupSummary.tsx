...
const getResultDisplayText = (
  resultDisplay: IMessageToolGroup["content"][0]["resultDisplay"],
): string | undefined => {
...
const ToolGroupMapper = (m: IMessageToolGroup): ToolItem[] => {
  return m.content.map(
    ({ name, callId, description, confirmationDetails, status, resultDisplay }) => {
      ...
      if (type === "mcp")
        desc = confirmationDetails.serverName + ":" + confirmationDetails.toolName;
  ...
  return (
    <div className="flex flex-col">
      <div className="flex flex-row color-#86909C gap-12px items-center">
        <Badge
          status={item.status}
          className={item.status === "processing" ? "badge-breathing" : ""}
        ></Badge>
        <span
          className={
            "flex-1 min-w-0" +
...
const MessageToolGroupSummary: React.FC<{
  ...
  return (
    <div>
      <div
        className="flex items-center gap-10px color-#86909C cursor-pointer"

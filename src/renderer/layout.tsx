...
const Layout: React.FC<{
  ...
  const loadAndHealCustomCss = useCallback(async () => {
    try {
      ...
      if (!effectiveCss && activeThemeId && activeThemeId !== "default-theme") {
        const defaultCss = resolveCssByActiveTheme(
          "default-theme",
          (savedThemes || []) as ICssTheme[],
        );
      ...
      if (lastCssRef.current !== effectiveCss) {
        ...
        window.dispatchEvent(
          new CustomEvent("custom-css-updated", { detail: { customCss: effectiveCss } }),
        );
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const handleStorageChange = (event: StorageEvent) => {
      if (
        event.key &&
        (event.key.includes("customCss") || event.key.includes("css.activeThemeId"))
      ) {
        void loadAndHealCustomCss();
      }
    };
  ...
  return (
    <LayoutContext.Provider
      value={{ isMobile, siderCollapsed: collapsed, setSiderCollapsed: setCollapsed }}
    >
      <div className="app-shell flex flex-col size-full min-h-0">
        ...
        <ArcoLayout className={"size-full layout flex-1 min-h-0"}>
          ...
          >
            ...
            <ArcoLayout.Content
              className={classNames(
                "p-8px layout-sider-content",
                !isMobile && "h-[calc(100%-72px-16px)]",
              )}
            >

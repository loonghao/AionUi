...
export const useGuidSend = (deps: GuidSendDeps): GuidSendResult => {
  ...
  const handleSend = useCallback(async () => {
    ...
    if (
      ...
      (isPreset && finalEffectiveAgentType === "gemini")
    ) {
      ...
      try {
        ...
        if (!conversation || !conversation.id) {
          throw new Error(
            "Failed to create conversation - conversation object is null or missing id",
          );
        }
        ...
        sessionStorage.setItem(
          `gemini_initial_message_${conversation.id}`,
    ...
    if (selectedAgent === "openclaw-gateway") {
      ...
      try {
        ...
        if (!conversation || !conversation.id) {
          alert(
            "Failed to create OpenClaw conversation. Please ensure the OpenClaw Gateway is running.",
          );
          return;
        }
        ...
        sessionStorage.setItem(
          `openclaw_initial_message_${conversation.id}`,
          JSON.stringify(initialMessage),
        );
    ...
    if (selectedAgent === "nanobot") {
      ...
      try {
        ...
        sessionStorage.setItem(
          `nanobot_initial_message_${conversation.id}`,
          JSON.stringify(initialMessage),
        );
    ...
    // ACP path (including preset with claude agent type)
    {
      ...
      if (!acpAgentInfo && !isPreset) {
        console.warn(
          `${acpBackend} CLI not found, but proceeding to let conversation panel handle it.`,
        );
      }
      ...
      try {
        const conversation = await ipcBridge.conversation.create.invoke({
          ...
          extra: {
            ...
            enabledSkills: isPreset ? enabledSkills : undefined,
            presetAssistantId: isPreset
              ? agentInfo?.customAgentId || acpAgentInfo?.customAgentId
              : undefined,
            sessionMode: selectedMode,
        ...
        if (!conversation || !conversation.id) {
          console.error(
            "Failed to create ACP conversation - conversation object is null or missing id",
          );
          return;
        }

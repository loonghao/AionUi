...
export type GuidModelSelectionResult = {
  ...
  geminiModeLookup: Map<
    string,
    ReturnType<typeof useGeminiGoogleAuthModels>["geminiModeOptions"][number]
  >;
...
export const useGuidModelSelection = (): GuidModelSelectionResult => {
  ...
  const geminiModelValues = useMemo(
    () => geminiModeOptions.map((option) => option.value),
    [geminiModeOptions],
  );
  ...
  const setCurrentModel = useCallback(async (modelInfo: TProviderWithModel) => {
    ...
    await ConfigStorage.set("gemini.defaultModel", {
      id: modelInfo.id,
  ...
  useEffect(() => {
    const setDefaultModel = async () => {
      ...
      const currentKey =
        selectedModelKeyRef.current || buildModelKey(currentModel?.id, currentModel?.useModel);
      ...
      if (isNewFormat) {
        ...
        resolvedUseModel = defaultModel?.model.includes(savedModel)
          ? savedModel

...
const ChatLayout: React.FC<{
  ...
  const { data: customAgents } = useSWR(
    backend === "custom" && !agentName ? "acp.customAgents" : null,
  ...
  const effectiveWorkspaceRatio =
    workspaceEnabled && isDesktop && !rightSiderCollapsed ? workspaceSplitRatio : 0;
  ...
  const desktopWorkspaceWidthPx = Math.min(
    500,
    Math.max(200, (workspaceSplitRatio / 100) * (viewportWidth || 0)),
  );
  const workspaceWidthPx = workspaceEnabled
    ? layout?.isMobile
  ...
  useEffect(() => {
    ...
    const maxWorkspace = Math.max(
      MIN_WORKSPACE_RATIO,
      Math.min(40, 100 - chatSplitRatio - MIN_PREVIEW_RATIO),
    );
  ...
  useEffect(() => {
    ...
    const maxChat = Math.max(
      MIN_CHAT_RATIO,
      Math.min(80, 100 - activeWorkspaceRatio - MIN_PREVIEW_RATIO),
    );
  ...
  useEffect(() => {
    ...
    if (isPreviewOpen && !previousPreviewOpenRef.current) {
      ...
      if (
        ...
        typeof layout?.siderCollapsed !== "undefined"
      ) {
  ...
  const mobileWorkspaceHandleRight = rightSiderCollapsed
    ? 0
    : Math.max(0, Math.round(workspaceWidthPx) - 14);
      ...
      >
        ...
        >
          {!layout?.isMobile && !hasTabs && (
            <span className="font-bold text-16px text-t-primary inline-block overflow-hidden text-ellipsis whitespace-nowrap max-w-full">
              {props.title}
            </span>
          )}
        </FlexFullContainer>
  ...
  return (
    ...
    >
      <div
        ref={containerRef}
        className={classNames(
          "flex flex-1 relative w-full overflow-hidden",
          useHeaderFullWidth && "flex-col",
        )}
      >
        {useHeaderFullWidth ? (
          <>
            ...
            <div className="flex flex-1 min-h-0 relative">
              <div
                ...
                onClick={() =>
                  layout?.isMobile && !rightSiderCollapsed && setRightSiderCollapsed(true)
                }
              >
              ...
              <div
                className="preview-panel flex flex-col relative overflow-hidden mt-[6px] mb-[12px] mr-[12px] ml-[8px] rounded-[15px]"
                style={{
                  flexGrow: 1,
                  flexShrink: 1,
                  flexBasis: 0,
              ...
              {workspaceEnabled && (
                <div
                  ...
                  style={{
                    flexGrow: 0,
                    flexShrink: 0,
                    flexBasis: rightSiderCollapsed ? "0px" : `${workspaceFlex}%`,
          ...
          <>
            ...
            {isPreviewOpen && (
              <div
                className={classNames(
                  "preview-panel flex flex-col relative overflow-hidden rounded-[15px]",

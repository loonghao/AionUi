    ...
    void ipcBridge.database.getUserConversations
      ...
      .then((conversations) => {
        const currentConversation = conversations?.find(
          (conv: TChatConversation) => conv.id === currentTab.id,
        );
  ...
  return (
    <div className="relative shrink-0 bg-2 min-h-40px">
      <div className="relative flex items-center h-40px w-full border-t border-x border-solid border-[color:var(--border-base)]">
        ...
        {/* 新建会话按钮 */}
        <div
          className="flex items-center justify-center w-40px h-40px shrink-0 cursor-pointer transition-colors duration-200 hover:bg-[var(--fill-2)] "

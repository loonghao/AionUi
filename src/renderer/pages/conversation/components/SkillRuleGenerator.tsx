...
const LoadRuleModal: React.FC<{
  ...
  const loadFiles = async () => {
    ...
    try {
      ...
      const flatList =
        result && result.length > 0 && result[0].children ? flattenFiles(result[0].children) : [];
      // Sort by name
      ...
      Message.error(
        t("conversation.skill_generator.load_error", { defaultValue: "Failed to load files" }),
      );
    } finally {
      setLoading(false);
    }
...
Please acknowledge receiving this rule/skill and confirm you will apply it.
      ...
      Message.success(
        t("conversation.skill_generator.rule_loaded", { defaultValue: "Rule loaded successfully" }),
      );
      onCancel();
    } catch (error) {
      console.error("Failed to read file:", error);
      Message.error(
        t("conversation.skill_generator.read_error", { defaultValue: "Failed to read file" }),
      );
    } finally {
...
const SkillRuleGenerator: React.FC<SkillRuleGeneratorProps> = ({ conversationId, workspace }) => {
  ...
  const handleGenerate = async () => {
    if (!workspace) {
      Message.error(
        t("conversation.skill_generator.no_workspace", { defaultValue: "No workspace available" }),
      );
      return;
    }
    ...
    if (!presetName.trim()) {
      Message.warning(
        t("conversation.skill_generator.name_required", {
          defaultValue: "Please enter a name for the preset",
        }),
      );
      return;
    }
    ...
    try {
      ...
      if (!messages || messages.length === 0) {
        Message.warning(
          t("conversation.skill_generator.no_history", {
            defaultValue: "No conversation history found",
          }),
        );
        setLoading(false);
        return;
...
- After saving, reply with a brief confirmation.
      ...
      setPresetName("");
      Message.success(
        t("conversation.skill_generator.request_sent", { defaultValue: "Request sent to agent" }),
      );
    } catch (error) {
      console.error("Failed to generate skill/rule:", error);
      Message.error(
        t("conversation.skill_generator.failed", { defaultValue: "Failed to generate" }),
      );
    } finally {
      setLoading(false);
    }
  ...
  const registerPreset = async (name: string, content: string) => {
    try {
      ...
      Message.success(
        t("conversation.skill_generator.preset_registered", {
          defaultValue: "Agent preset registered successfully!",
        }),
      );
    } catch (error) {
    ...
    <Menu>
      <Menu.Item key="generate" onClick={() => setGenerateVisible(true)}>
        <div className="flex items-center gap-2">
          <Magic />
          {t("conversation.skill_generator.menu_generate", {
            defaultValue: "Generate from History",
          })}
        </div>
      </Menu.Item>
    ...
    <>
      ...
      >
        ...
        <Radio.Group value={type} onChange={setType}>
          ...
          <Radio value="rule">
            {t("conversation.skill_generator.type_rule", { defaultValue: "Rule (JSON/MD)" })}
          </Radio>
        </Radio.Group>
      </Modal>

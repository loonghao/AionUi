...
const AssistantManagement: React.FC<AssistantManagementProps> = ({ message }) => {
  ...
  const loadAssistantSkills = useCallback(
    async (assistantId: string): Promise<string> => {
      try {
        const content = await ipcBridge.fs.readAssistantSkill.invoke({
          assistantId,
          locale: localeKey,
        });
        return content || "";
  ...
  const handleDuplicate = async (assistant: AcpBackendConfig) => {
    ...
    try {
      const [context, skills, skillsList] = await Promise.all([
        loadAssistantContext(assistant.id),
        loadAssistantSkills(assistant.id),
  ...
  const handleSave = async () => {
    try {
      ...
      if (!editName.trim()) {
        message.error(
          t("settings.assistantNameRequired", { defaultValue: "Assistant name is required" }),
        );
        return;
      }
      ...
      if (pendingSkills.length > 0) {
        ...
        if (skillsToImport.length > 0) {
          for (const pendingSkill of skillsToImport) {
            try {
              const response = await ipcBridge.fs.importSkill.invoke({
                skillPath: pendingSkill.path,
              });
              if (!response.success) {
      ...
      if (isCreating) {
        ...
        const updatedAgents = agents.map((agent) =>
          agent.id === activeAssistant.id ? updatedAgent : agent,
        );
        await ConfigStorage.set("acp.customAgents", updatedAgents);
        setAssistants(sortAssistants(updatedAgents));
  ...
  const handleDeleteClick = () => {
    ...
    if (activeAssistant.isBuiltin) {
      message.warning(
        t("settings.cannotDeleteBuiltin", { defaultValue: "Cannot delete builtin assistants" }),
      );
      return;
    }
  ...
  const handleToggleEnabled = async (assistant: AcpBackendConfig, enabled: boolean) => {
    try {
      const agents = (await ConfigStorage.get("acp.customAgents")) || [];
      const updatedAgents = agents.map((agent) =>
        agent.id === assistant.id ? { ...agent, enabled } : agent,
      );
      await ConfigStorage.set("acp.customAgents", updatedAgents);
    ...
    <div>
          ...
          <div className="flex flex-col flex-1 gap-16px bg-fill-2 rounded-16px p-20px overflow-y-auto">
              ...
              <div className="mt-10px flex items-center gap-12px">
                {activeAssistant?.isBuiltin ? (
                  <Avatar shape="square" size={40} className="bg-bg-1 rounded-4px">
                    {editAvatar && avatarImageMap[editAvatar.trim()] ? (
                      <img
                        src={avatarImageMap[editAvatar.trim()]}
                        alt=""
                        width={24}
                        height={24}
                        style={{ objectFit: "contain" }}
                  ...
                  <EmojiPicker
                    value={editAvatar}
                    onChange={(emoji) => setEditAvatar(emoji)}
                    placement="br"
                  >
            ...
            <div className="flex-shrink-0">
              ...
              <Select
                ...
                value={editAgent}
                onChange={(value) => setEditAgent(value as PresetAgentType)}
              >
            ...
            <div className="flex-shrink-0">
              ...
              >
                ...
                <div
                  className="bg-fill-2"
                  style={{
                    height: activeAssistant?.isBuiltin ? "100%" : "calc(100% - 36px)",
                    overflow: "auto",
                  }}
                >
            ...
            {(isCreating ||
              ...
              (activeAssistant && !activeAssistant.isBuiltin)) && (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-4px">
                              ...
                              <span className="text-10px px-4px py-1px bg-primary-1 text-primary rounded">
                                Pending
                              </span>
                            </div>
                            {skill.description && (
                        ...
                        .map((skill) => (
                          ...
                          >
                            ...
                            <button
                              ...
                              title={t("settings.removeFromAssistant", {
                                defaultValue: "Remove from assistant",
                              })}
                            >
                              <Delete size={16} fill="var(--color-text-3)" />
                            </button>
                        ...
                        availableSkills.filter((skill) => skill.isCustom).length === 0 && (
                          <div className="text-center text-t-secondary text-12px py-16px">
                            {t("settings.noCustomSkills", {
                              defaultValue: "No custom skills added",
                            })}
                          </div>
                        )}
                    </div>
      ...
      <Modal
        ...
        onOk={async () => {
          if (!skillPath.trim()) {
            message.warning(
              t("settings.pleaseSelectSkillPath", {
                defaultValue: "Please select a skill folder path",
              }),
            );
            return;
          }
          ...
          try {
            ...
            if (allFoundSkills.length > 0) {
              ...
              for (const skill of allFoundSkills) {
                ...
                const alreadyInAssistant =
                  customSkills.includes(name) || newCustomSkillNames.includes(name);
              ...
              if (addedCount > 0) {
                ...
                message.warning(
                  t("settings.allSkillsExist", { defaultValue: "All found skills already exist" }),
                );
              }
              ...
              message.warning(
                t("settings.noSkillsFound", {
                  defaultValue: "No valid skills found in the selected path(s)",
                }),
              );
              setSkillsModalVisible(false);
            }
      ...
      >
        <div className="space-y-16px">
          ...
          <div className="space-y-12px">
            <Typography.Text>
              {t("settings.skillFolderPath", { defaultValue: "Skill Folder Path" })}
            </Typography.Text>
      ...
      <Modal
        ...
        onOk={() => {
          if (deletePendingSkillName) {
            ...
            message.success(
              t("settings.skillDeleted", { defaultValue: "Skill removed from pending list" }),
            );
          }
        }}
        className="w-[90vw] md:w-[400px]"
      ...
      <Modal
        ...
        title={t("settings.removeCustomSkillTitle", {
          defaultValue: "Remove Skill from Assistant",
        })}
        okButtonProps={{ status: "danger" }}
        ...
        onOk={() => {
          if (deleteCustomSkillName) {
            ...
            message.success(
              t("settings.skillRemovedFromAssistant", {
                defaultValue: "Skill removed from this assistant",
              }),
            );
          }
        }}

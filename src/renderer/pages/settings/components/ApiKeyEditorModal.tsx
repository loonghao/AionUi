 ...
 */
const ApiKeyEditorModal: React.FC<ApiKeyEditorModalProps> = ({
  visible,
  apiKeys,
  onClose,
  onSave,
  ...
  const executeKeyTest = useCallback(
    async (id: string, value: string) => {
      ...
      try {
        ...
        setKeys((prev) =>
          prev.map((k) => (k.id === id ? { ...k, status: isValid ? "valid" : "invalid" } : k)),
        );
      } catch {
                ...
                <div className="flex items-center gap-4px shrink-0">
                  {key.editing ? (
                    ...
                    <>
                      {/* 状态图标 - 在测试按钮左边 */}
                      {key.status !== "pending" && (
                        <div className="flex items-center">{getStatusIcon(key.status)}</div>
                      )}

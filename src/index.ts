...
import './utils/configureConsole';
import './utils/configureChromium';
import { cdpPort, verifyCdpReady, getActiveCdpInstances, unregisterInstance } from './utils/configureChromium';
import { app, BrowserWindow, nativeImage, nativeTheme, powerMonitor, protocol, screen, net } from 'electron';
...
// electron-vite provides ELECTRON_RENDERER_URL in dev mode for the renderer dev server URL.
...
// Register custom protocol scheme for serving local extension assets.
// In dev mode, the renderer loads from http://localhost:5173/ which blocks file:// URLs.
// The aion-asset:// protocol bypasses this restriction by serving files through Electron's net module.
...
process.on('uncaughtException', (error) => {
  ...
  if (app.isPackaged) {
    // In production, force exit to prevent undefined state
    process.exit(1);
  }
  // In development, tolerate non-fatal network errors (e.g., port conflicts)
  // to avoid crashing the entire app during iterative development
  const errno = (error as NodeJS.ErrnoException).code;
  if (errno === 'EADDRINUSE' || errno === 'EACCES') {
    console.warn(`[App] Non-fatal error (${errno}) in dev mode, continuing...`);
    return;
  }
  // For other uncaught exceptions in dev, still exit to prevent hangs
  process.exit(1);
});
...
process.on('unhandledRejection', (reason, _promise) => {
  ...
  // Force exit on unhandled rejection to prevent hanging
  process.exit(1);
});
...
const createWindow = (): void => {
  ...
  // Use show:false + ready-to-show to eliminate the blank/black flash on startup.
  // Pick a background color that matches the system theme so the brief moment
  // before the renderer paints looks seamless.
  ...
  // Show window only after the first paint to avoid blank/black flash.
  // Fallback timeout ensures the window appears even if ready-to-show doesn't fire.
...
application.openDevTools.provider(() => {
  if (mainWindow) {
    mainWindow.webContents.openDevTools();
  }
...
const handleAppReady = async (): Promise<void> => {
  // Register the aion-asset:// protocol handler to serve local extension assets.
  // URL format: aion-asset://asset/C:/path/to/file — uses pathToFileURL for correct file:// conversion.
  // This must be called after app.whenReady().
  protocol.handle(AION_ASSET_PROTOCOL, (request) => {
    const { pathname } = new URL(request.url);
    const decoded = decodeURIComponent(pathname);
    // On Windows, pathname starts with /C:/... — strip the leading slash before the drive letter
    // so pathToFileURL produces file:///C:/... instead of file:///C:/C:/...
    const filePath = /^\/[A-Za-z]:/.test(decoded) ? decoded.slice(1) : decoded;
    return net.fetch(pathToFileURL(filePath).toString());
  });

  // Log CDP remote debugging info and verify connectivity for chrome-devtools-mcp integration
  if (cdpPort) {
    console.log(`[DevTools MCP] Remote debugging configured on port ${cdpPort} (PID: ${process.pid})`);
    // Async health check — do not block app startup
    verifyCdpReady(cdpPort).then((ready) => {
      if (ready) {
        console.log(`[DevTools MCP] ✅ CDP ready on port ${cdpPort}`);
        console.log(`[DevTools MCP] Connect: npx chrome-devtools-mcp@latest --browserUrl=http://localhost:${cdpPort}`);
        // Show all active instances from the registry
        const instances = getActiveCdpInstances();
        if (instances.length > 1) {
          console.log(`[DevTools MCP] 📋 Active instances (${instances.length}):`);
          for (const inst of instances) {
            const marker = inst.pid === process.pid ? ' ← current' : '';
            console.log(`[DevTools MCP]   PID ${inst.pid} → port ${inst.port} (${inst.cwd})${marker}`);
          }
        }
      } else {
        console.warn(`[DevTools MCP] ⚠️  CDP port ${cdpPort} is NOT accessible — port may be occupied by a non-AionUi process.`);
        console.warn(`[DevTools MCP] Try: set AIONUI_CDP_PORT=<port> or kill the process occupying port ${cdpPort}.`);
      }
    });
  }
  ...
  // Initialize extension system FIRST, before initializeProcess/initStorage,
  // so that extension contributions (assistants, skills, etc.) are available
  // when initStorage merges them into the config.
  ...
  if (isResetPasswordMode) {
    ...
    if (isWebUIMode) {
      ...
      // In desktop mode, auto-start embedded web server if extensions have WebUI contributions
      // 桌面模式下，如果扩展有 WebUI 贡献，自动启动内嵌 web server
      try {
        const { ExtensionRegistry } = await import('./extensions');
        const webuiContributions = ExtensionRegistry.getInstance().getWebuiContributions();
        if (webuiContributions.length > 0) {
          ...
          try {
            const instance = await startWebServerWithInstance(embeddedPort, false);
            setWebServerInstance(instance);
            console.log(`[App] Embedded web server started on port ${embeddedPort} for extension WebUI`);
          } catch (err) {
            // Port may be in use; log but don't block app startup
            console.warn(`[App] Failed to start embedded web server (port ${embeddedPort} may be in use):`, err);
          }
        }
      } catch (err) {
        console.warn('[App] Failed to check extension WebUI contributions:', err);
      }
    }

    // Lazy-load ACP detector to avoid pulling in entire agent subtree at build time
    ...
    import('./process/utils/shellEnv').then(({ loadShellEnvironmentAsync }) => {
...
app.on('before-quit', async () => {
  // 在应用退出前清理工作进程
  // Use a timeout to ensure we don't hang indefinitely during cleanup
  const cleanupTimeout = setTimeout(() => {
    console.log('[App] Cleanup timeout, forcing exit');
    process.exit(0);
  }, 3000);

  // Clean up CDP registry entry for this instance
  unregisterInstance();
